...
function isVolanteUnit(u: Unit) { return u.subtype === "volante"; }
...
function getScorer(category: Category, tab: Tab, maxSpeed: number) {
  if (tab === "attaque") {
    ...
    if (category === "volante") return (u: Unit) => attackPerPop(u) * (1 + u.speed / maxSpeed * 0.5);
    return (u: Unit) => attackPerPop(u) * (ATK_WEIGHT[category][u.attackType ?? "mixte"] ?? 1.0);
  }
  switch (category) {
...
function UnitCard({ unit, tab, maxAtk, maxDef }: { unit: Unit; tab: Tab; maxAtk: number; maxDef: number }) {
  const { t } = useTheme();
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          <div style={{ marginTop: 4, background: t.lavender, border: `1px solid ${t.lavenderMid}`, borderRadius: 7, padding: "5px 8px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
              ...
              <img src={isNavale ? ICONS.defCoup : specialty === "Contondant" ? ICONS.defCoup : specialty === "Tranchant" ? ICONS.defLame : ICONS.defJet} alt="" style={{ width: 14, height: 14, objectFit: "contain" }} />
              <span style={{ fontSize: 9, color: t.textDim }}>par</span>
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
        {filtered.map((u) => <UnitCard key={u.id} unit={u} tab={tab} maxAtk={maxAtk} maxDef={maxDef} />)}
      </div>
    </div>
  );
}
...
export function TroupesClient() {
  ...
  const visibleUnits = useMemo(() => filterUnits(UNITS, category), [category]);
  const siegeUnits = useMemo(() => UNITS.filter((u) => u.isSiege), []);
  const scorer = useMemo(() => getScorer(category, tab, maxSpeed), [category, tab, maxSpeed]);
  const tiers = useMemo(() => assignTiers(visibleUnits, scorer), [visibleUnits, scorer]);
  const maxAtk = useMemo(() => Math.max(...visibleUnits.map((u) => u.attack), 1), [visibleUnits]);
  const maxDef = useMemo(() => Math.max(...visibleUnits.flatMap((u) => [u.defContond, u.defBlanche, u.defJet]), 1), [visibleUnits]);
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* Header */}
      <div style={{ padding: "14px 24px 12px", borderBottom: `1px solid ${t.border}`, background: t.bgCard, flexShrink: 0, display: "flex", alignItems: "center" }}>
        <div style={{ flex: 1 }}>
      ...
      {/* Tabs + Categories */}
      <div style={{ padding: "10px 24px", borderBottom: `1px solid ${t.border}`, background: t.bgCard, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
        {visibleUnits.length === 0 ? (
          <div style={{ color: t.textDim, fontSize: 13, textAlign: "center", marginTop: 40 }}>Aucune unité dans cette catégorie</div>
        ) : (
          TIER_ORDER.map((tier) => (
        ...
        {showSiege && siegeUnits.length > 0 && (
          <div style={{ marginTop: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <div style={{ borderRadius: 9, background: "#37415122", border: "2px solid #374151", display: "flex", alignItems: "center", padding: "4px 10px", fontWeight: 700, fontSize: 12, color: "#6b7280", gap: 6, flexShrink: 0 }}>
                🏰 Siège
              </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {siegeUnits.map((u) => <UnitCard key={u.id} unit={u} tab={tab} maxAtk={maxAtk} maxDef={maxDef} />)}
            </div>
          </div>
        )}
      </div>
